...
public class Equalizer {
	...
	public Equalizer(PApplet p, int x, int y, int z, Fourier f) {
		...
		this.hauteur = 100;
		coefs = new float[f.getCentre().length];
	...
	public void creerBarre(int i) {
		...
		parent.stroke(255);
		parent.rect(i*largeurunit, hauteur * (1-coefs[i]), largeurunit, hauteur * coefs[i]);
	...
	public void sourisDetect() {
		...
		parent.stroke(255);
		parent.rect(this.positionX, this.positionY, this.longtotale, 1*hauteur);
