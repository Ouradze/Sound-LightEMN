...
public class StripeAnal extends Design{
	...
	public StripeAnal(PApplet p,  int x, int y) {
		super(p,x,y, 0);
	...
	public void display() {
		...
		for (int i = 1; i < this.freqcentre.length; i++) {
			...
			parent.fill(0);
			parent.stroke(255);
			parent.rect( i * (largeur + espace), -freqcentre[i], largeur, // cr�e un rectangle
					2 * freqcentre[i] + 2);
	...
	public void scale() {
		...
		anal.maj(this.f.getFreqBrute(), f.getSampleRate());
		float[] freq = CalculFourier.CutScaling(Lissage.Lissercubique(this.anal.getFFTepure()), (float) 0.8);
